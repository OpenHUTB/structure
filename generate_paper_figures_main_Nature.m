...
ax1 = axes('Position', ...
    [init_x init_y+factor_y*length_y*(2-1) length_x length_y]);    
patch(ax1, 'Vertices', surface_downsampled.vertices, ...
    'Faces', surface_downsampled.faces, 'FaceVertexCData', data_to_plot, ...
           'EdgeColor', 'k', ...
           'FaceColor', 'interp', ...
...
colormap(ax5_1,[0.7*ones(1,3); 0.7*ones(1,3); colors3])
axis off
axis image
...
colormap(ax5_2,[0.7*ones(1,3); 0.7*ones(1,3); colors3])
axis off
axis image
...
text(min(get(gca,'xlim'))+0.05*[max(get(gca,'xlim'))-min(get(gca,'xlim'))], min(get(gca,'ylim'))+0.12*[max(get(gca,'ylim'))-min(get(gca,'ylim'))], sprintf('p_{spin} = %.3f', pval), 'color', 'r', ...
    'fontsize', fontsize_axis, 'fontweight', 'bold', 'verticalalignment', 'middle', 'horizontalalignment', 'left');
box off
